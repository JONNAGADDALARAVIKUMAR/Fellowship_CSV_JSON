	...
	private static void addToCsv(HashMap<String, HashMap> addressBooks) throws IOException { //Writes Data to the CSV file
		...
		try (
	        ...
	        for(java.util.Map.Entry<String, HashMap> eachAddressBook : addressBooks.entrySet()) {
	        	 ...
	        	 HashMap<String, DetailsCollector> contacts = eachAddressBook.getValue();
			
	        	 for(DetailsCollector details : contacts.values()) {
	        		 csvWriter.writeNext(new String[] {(String) addressBookName, details.getFirstName(), details.getLastName(), 
